...
-- Incluye CREATE TABLE IF NOT EXISTS por si la migración
-- original falló silenciosamente en producción.
-- ============================================================
...
BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE schemaname='public' AND tablename='knowledge_blocks' AND policyname='Users can view own knowledge_blocks') THEN
    CREATE POLICY "Users can view own knowledge_blocks" ON public.knowledge_blocks FOR SELECT USING (auth.uid() = user_id);
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE schemaname='public' AND tablename='knowledge_blocks' AND policyname='Users can insert own knowledge_blocks') THEN
    CREATE POLICY "Users can insert own knowledge_blocks" ON public.knowledge_blocks FOR INSERT WITH CHECK (auth.uid() = user_id);
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE schemaname='public' AND tablename='knowledge_blocks' AND policyname='Users can update own knowledge_blocks') THEN
    CREATE POLICY "Users can update own knowledge_blocks" ON public.knowledge_blocks FOR UPDATE USING (auth.uid() = user_id);
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE schemaname='public' AND tablename='knowledge_blocks' AND policyname='Users can delete own knowledge_blocks') THEN
    CREATE POLICY "Users can delete own knowledge_blocks" ON public.knowledge_blocks FOR DELETE USING (auth.uid() = user_id);
  END IF;
END $setup$;

CREATE UNIQUE INDEX IF NOT EXISTS idx_knowledge_blocks_source_hash
  ON public.knowledge_blocks (user_id, source_hash)
