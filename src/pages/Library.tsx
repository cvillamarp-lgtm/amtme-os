              ...
              <CardContent className="p-3 space-y-2">
                ...
                <div className="flex gap-1" onClick={(e) => e.stopPropagation()}>
                  {asset.status === "generated" && (
                    <Button size="sm" variant="outline" className="h-10 text-xs flex-1" onClick={() => updateStatus(asset.id, "approved")}>
                      <CheckCircle2 className="h-3 w-3 mr-1" />
                      Aprobar
                    </Button>
                  )}
                  {asset.status === "approved" && (
                    <Button size="sm" variant="outline" className="h-10 text-xs flex-1" onClick={() => updateStatus(asset.id, "published")}>
                      Publicar
                    </Button>
                  )}
                  {asset.caption && (
                    <Button size="sm" variant="ghost" className="h-10 w-10 p-0" onClick={() => copyCaption(asset)}>
                      {copiedId === asset.id ? <Check className="h-3 w-3" /> : <Copy className="h-3 w-3" />}
                    </Button>
                  )}
                  {asset.image_url && (
                    <Button size="sm" variant="ghost" className="h-10 w-10 p-0" asChild>
                      <a href={asset.image_url} download target="_blank" rel="noopener noreferrer">
                        <Download className="h-3 w-3" />
                      </a>
                    </Button>
                  )}
                  <Button size="sm" variant="ghost" className="h-10 w-10 p-0 text-destructive" onClick={() => deleteAsset(asset.id)}>
