            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                ...
                <div className="space-y-2 text-sm">
                  <div className="flex justify-between">
                    ...
                    <span className="font-mono text-foreground">
                      {selectedPiece.dimensiones.pixels}
                    </span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-muted-foreground">Ratio</span>
                    <span className="font-mono text-foreground">
                      {selectedPiece.dimensiones.ratio}
                    </span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-muted-foreground">Grid</span>
                    <span className="text-foreground">{selectedPiece.dimensiones.grid}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-muted-foreground">Gutters</span>
                    <span className="font-mono text-foreground">
                      {selectedPiece.dimensiones.gutters}
                    </span>
                  </div>
                </div>
              </div>
              <div className="surface p-5 space-y-3">
                ...
                <p className="text-sm font-mono text-foreground">
                  {selectedPiece.dimensiones.safeZone.label}
                </p>
                ...
                <div className="space-y-1">
                  {(selectedPiece.dimensiones.noGoZones as NoGoZone[]).map((z, i: number) => (
                    <div
                      key={i}
                      className="text-xs font-mono text-destructive/80 bg-destructive/5 px-2 py-1 rounded"
                    >
                      {z.area}: {z.condition}
                    </div>
                  ))}
            ...
            <div className="space-y-4">
              ...
              <div className="surface p-5">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  {Object.entries(
                    selectedPiece.estructura.coordenadas as Record<string, Coord>
                  ).map(([key, coord]) => (
              ...
              <div className="surface p-5">
                ...
                <p className="text-sm text-muted-foreground">
                  {selectedPiece.estructura.composicion}
                </p>
                <h4 className="text-xs font-semibold text-muted-foreground mt-3 mb-1">
                  Flujo de lectura
                </h4>
            ...
            <div className="surface p-5 space-y-4">
              ...
              <div className="space-y-2 text-sm">
                <div className="flex justify-between">
                  ...
                  <span className="text-foreground">{selectedPiece.tipografia.h1.fuente}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-muted-foreground">Peso</span>
                  <span className="text-foreground">{selectedPiece.tipografia.h1.peso}</span>
                </div>
                {selectedPiece.tipografia.h1.tamano && (
                  <div className="flex justify-between">
                    <span className="text-muted-foreground">Tamaño</span>
                    <span className="font-mono text-foreground">
                      {selectedPiece.tipografia.h1.tamano}pt
                    </span>
                  </div>
                )}
                <div className="flex justify-between">
                  ...
                  <div className="flex items-center gap-2">
                    <div
                      ...
                      style={{ backgroundColor: selectedPiece.tipografia.h1.color }}
                    />
                    <span className="font-mono text-foreground">
                      {selectedPiece.tipografia.h1.color}
                    </span>
                  </div>
                </div>
              </div>
              <div className="pt-2 border-t border-border">
                <div className="flex justify-between text-sm">
                  ...
                  <span className="text-foreground">{selectedPiece.tipografia.jerarquia}</span>
                </div>
                <div className="flex justify-between text-sm mt-1">
                  <span className="text-muted-foreground">Legibilidad móvil</span>
                  <span className="text-foreground">
                    {selectedPiece.tipografia.legibilidadMovil}
                  </span>
                </div>
              </div>
            </div>
              ...
              <div className="pt-3 border-t border-border space-y-2 text-sm">
                <div className="flex justify-between">
                  ...
                  <span className="text-foreground">
                    {String(
                      (
                        selectedPiece.paleta.contraste as
                          | { ratio?: string | number; nivel?: string }
                          | undefined
                      )?.ratio ?? ""
                    )}
                    :1 (
                ...
                <div className="flex justify-between">
                  <span className="text-muted-foreground">Acentos</span>
                  <span className="text-foreground">{selectedPiece.paleta.acentos}</span>
                </div>
              </div>
            </div>
          </TabsContent>
              ...
              <div className="pt-3 border-t border-border">
                ...
                <div className="space-y-1">
                  {selectedPiece.elementos.reglasNoEdicion.map((r: string, i: number) => (
                    <div
                      key={i}
                      className="text-xs text-destructive/80 bg-destructive/5 px-3 py-1.5 rounded"
                    >
                      {r}
                    </div>
                  ))}
    ...
    <div className="page-container animate-fade-in">
      <div>
        ...
        <p className="page-subtitle">
          Sistema de Especificaciones Visuales AMTME — {data.piezas.length} piezas analizadas
        </p>
      </div>
              ...
              {filteredPieces.map((piece) => (
                ...
                >
                  ...
                  <div className="flex items-center justify-between text-[10px] text-muted-foreground">
                    ...
                    <span>
                      {piece.qa.score}/{piece.qa.total} QA
                    </span>
                  </div>
                </button>
              ))}
          ...
          <div className="surface p-5">
            <div className="flex items-center justify-between mb-3">
              ...
              <span className="text-xs text-muted-foreground">
                {Math.round(qaProgress)}% completado
              </span>
            </div>
                ...
                >
                  <Checkbox
                    ...
                    onCheckedChange={(checked) =>
                      setQaChecks({ ...qaChecks, [item.id]: !!checked })
                    }
                    className="mt-0.5"
                  />
                  <div>
        ...
        <TabsContent value="contrast" className="space-y-4">
          <div className="surface p-5 space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label htmlFor="contrastFg" className="text-xs text-muted-foreground mb-1 block">
              ...
              <div>
                <label htmlFor="contrastBg" className="text-xs text-muted-foreground mb-1 block">
            ...
            <div className="grid grid-cols-3 gap-3">
              ...
              <div
                className={`surface p-4 text-center ${meetsWCAG(contrastRatio, "AA") ? "border-[hsl(var(--chart-2)/0.3)]" : "border-destructive/30"}`}
              >
                <div
                  className={`text-lg font-bold ${meetsWCAG(contrastRatio, "AA") ? "text-[hsl(var(--chart-2))]" : "text-destructive"}`}
                >
              ...
              <div
                className={`surface p-4 text-center ${meetsWCAG(contrastRatio, "AAA") ? "border-[hsl(var(--chart-2)/0.3)]" : "border-destructive/30"}`}
              >
                <div
                  className={`text-lg font-bold ${meetsWCAG(contrastRatio, "AAA") ? "text-[hsl(var(--chart-2))]" : "text-destructive"}`}
                >
          ...
          {data.tipos.map((tipo) => (
            <div key={tipo.id} className="surface p-5 space-y-3">
              <div className="flex items-start justify-between">
                ...
                <Badge
                  variant={tipo.fondoRegla.decision.includes("BLANCO") ? "secondary" : "default"}
                  className="text-[10px]"
                >
              ...
              <div className="flex items-center gap-4 text-xs text-muted-foreground">
                <span className="font-mono">
                  {tipo.dimensiones.width}×{tipo.dimensiones.height}
                </span>
